...
class GetMatchesByLeagueInput(BaseModel):
    league: SportLeagueOutputModel = Field(
        ..., description="The league for which to fetch matches"
    )
    ...
    number_of_matches: Optional[int] = Field(
        10, description="Number of matches to return, default is 10"
    )
...
class GetMatchesByTeamInput(BaseModel):
    external_team_id: int = Field(
        ..., description="The external ID of the team for which to fetch matches"
    )
    kickoff_date: Union[None, date] = Field(
        None,
        description="Optional kick-off date to filter matches (matches on the same day as this date)",
...
class GetMatchesByTeamList(BaseModel):
    external_team_ids: List[int] = Field(
        ..., description="List of external team IDs for which to fetch matches"
    )
    kickoff_date: Union[None, date] = Field(
        None,
        description="Optional kick-off date to filter matches (matches on the same day as this date)",
    ...
    number_of_matches: Optional[int] = Field(
        30, description="Number of random matches to return, default is 30"
    )
...
class RandomMatchInput(BaseModel):
    ...
    number_of_matches: Optional[int] = Field(
        20, description="Number of random matches to return, default is 20"
    )
...
class HeadToHeadInput(BaseModel):
    ...
    number_of_matches: Optional[int] = Field(
        30, description="Number of matches to return, default is 30"
    )
...
def get_head_to_head_matches(
    ...
    query = _add_date_filters_if_needed(
        query, query_date, future_only=head_to_head_input.future_only
    )
...
def get_random_matches(
    ...
    logger.info(
        f"Fetching a random match with query date: {query_date}, future_only: {future_only}"
    )

    query = SportMatch.objects.select_related(
    ...
    matches = list(
        ...
        .exclude(metadata={})
        .order_by("kickoff_datetime")[: random_match_input.number_of_matches]
    )
    if not matches:
        logger.warning("No matches found for the external IDs in allowed_league_ids")
        new_qs = SportMatch.objects.select_related(
            "league__country", "home_team", "away_team"
        )
        new_qs = _add_date_filters_if_needed(new_qs, query_date, future_only)
        matches = list(
            new_qs.filter(type=ApiSportModel.SportType.SOCCER)
            .exclude(metadata={})
            .order_by("kickoff_datetime")[: random_match_input.number_of_matches]
        )
...
def get_matches_by_league(
    ...
    query = SportMatch.objects.filter(
        league__external_id=league.external_id, type=ApiSportModel.SportType.SOCCER
    ).select_related("league__country", "home_team", "away_team")
...
def get_matches_by_team(
    ...
    query = SportMatch.objects.filter(
        Q(home_team__external_id=external_team_id)
        | Q(away_team__external_id=external_team_id),
        type=ApiSportModel.SportType.SOCCER,
    ).select_related("league__country", "home_team", "away_team")
...
def get_matches_by_team_list(
    ...
    query = SportMatch.objects.filter(
        Q(home_team__external_id__in=external_team_ids)
        | Q(away_team__external_id__in=external_team_ids),
        type=ApiSportModel.SportType.SOCCER,
    ).select_related("league__country", "home_team", "away_team")
...
def get_match_insights_by_external_id(
    ...
    match = (
        SportMatch.objects.filter(
            external_id=external_id, type=ApiSportModel.SportType.SOCCER
        )
        .select_related("league__country", "home_team", "away_team")
        .first()
    )
...
def _add_date_filters_if_needed(
    ...
    logger.info(
        f"Current datetime: {timezone.now()}, Timezone: {timezone.get_current_timezone()}"
    )
