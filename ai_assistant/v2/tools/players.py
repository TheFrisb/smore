...
class PlayerInput(BaseModel):
    name_query: Optional[str] = Field(
        None, description="Name or partial name of the player to search for."
    )
    external_id: Optional[int] = Field(
        None,
        description="External ID of the player to fetch specific player information.",
    )
        ...
        player = (
            Player.objects.annotate(
                similarity=TrigramSimilarity(
                    Lower(Unaccent("name")),
                    Lower(Unaccent(Value(player_input.name_query))),
                )
            )
            .filter(similarity__gt=0.5, type=ApiSportModel.SportType.TEMP_FIX)
            .order_by("-similarity")
            .first()
        )
