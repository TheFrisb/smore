...
class InjuryOutputModel(BaseModel):
    ...
    injury_type: str = Field(
        ...,
        description="Type of injury sustained by the player."
    )
...
def get_match_injuries(match_injury_input: MatchInjuryInput) -> Optional[list[TeamWithInjuryOutputModel]]:
    ...
    """
    sport_match = SportMatch.objects.filter(external_id=match_injury_input.external_match_id,
                                            type=ApiSportModel.SportType.SOCCER).first()
    if not sport_match:
        return None
