...
class SportCountryOutputModel(BaseModel):
    ...
    code: Optional[str] = Field(
        None, max_length=255, description="The country code (optional)"
    )
...
class SportLeagueOutputModel(BaseModel):
    ...
    country: SportCountryOutputModel = Field(
        ..., description="The country associated with the league"
    )
...
class SportMatchOutputModel(BaseModel):
    ...
    home_team: SportTeamOutputModel = Field(
        ..., description="The home team in the match"
    )
    home_team_score: Optional[str] = Field(
        None, description="The score of the home team in the match, if available"
    )
    away_team_score: Optional[str] = Field(
        None, description="The score of the away team in the match, if available"
    )
    away_team: SportTeamOutputModel = Field(
        ..., description="The away team in the match"
    )
    league: SportLeagueOutputModel = Field(
        ..., description="The league the match belongs to"
    )
    kickoff_datetime: datetime = Field(
        ..., description="The date and time of the match kickoff"
    )
...
class PredictionStatisticsOutputModel(BaseModel):
    ...
    suggested_team_winner: Optional[SportTeamOutputModel] = Field(
        None, description="The team predicted to win the match, if applicable"
    )
...
class SportMatchInsightOutputModel(BaseModel):
    match: SportMatchOutputModel = Field(
        ..., description="The match for which the insight is provided"
    )
    ...
    prediction_statistics: Optional[PredictionStatisticsOutputModel] = Field(
        None, description="Prediction statistics for the match, if available"
    )
...
class PlayerOutputModel(BaseModel):
    ...
    full_name: str = Field(
        ..., max_length=255, description="The full name of the player"
    )
    first_name: Optional[str] = Field(
        None, description="The first name of the player, if available"
    )
    last_name: Optional[str] = Field(
        None, description="The last name of the player, if available"
    )
    age: Optional[int] = Field(None, description="The age of the player, if available")
    birth_date: Optional[str] = Field(
        None, description="The birth date of the player, if available"
    )
    birth_place: Optional[str] = Field(
        None, description="The birth place of the player, if available"
    )
    birth_country: Optional[str] = Field(
        None, description="The birth country, if available"
    )
    nationality: Optional[str] = Field(
        None, description="The nationality of the player, if available"
    )
...
class TeamStandingOutputModel(BaseModel):
    ...
    def from_django(cls, team_standing: TeamStanding) -> "TeamStandingOutputModel":
        ...
        data = {
            "league": SportLeagueOutputModel.model_validate(
                team_standing.league_team.league
            ),
            ...
            "total_goals_conceded": team_standing.data.get("all")
            .get("goals")
            .get("against"),
            ...
            "home_goals_conceded": team_standing.data.get("home")
            .get("goals")
            .get("against"),
            "away_goals_conceded": team_standing.data.get("away")
            .get("goals")
            .get("against"),
        }
