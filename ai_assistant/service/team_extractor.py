...
class TeamExtractor:
    def __init__(self, llm):
        ...
        self.prompt = PromptTemplate(
            ...
            template="""
            You are an AI assistant tasked with extracting the full names of sports teams from a conversation. Given the conversation history and the current message, identify the teams being referred to in the current message. If the message refers to a match, provide the full names of the teams involved in that match.
            ...
            **Instructions:**
            ...
            - History: "User: Who’s the best player? AI: Messi is great."
              Current Message: "Tell me about his team."
    ...
    def extract(
        ...
        try:
            ...
            logger.info(
                f"Extracting teams with message: '{message}' and history: '{history_text}'"
            )

            # Invoke the LLM with the prompt
            response = self.chain.invoke({"history": history_text, "message": message})
            extracted_names = [
