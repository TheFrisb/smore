...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--media-root", type=str, default="", help="Base media path for logo files"
        )
        ...
        for i, league in enumerate(leagues_data, 1):
            ...
            if i % BATCH_SIZE == 0 or i == total:
                SportLeague.objects.bulk_create(
                    leagues_to_create, batch_size=BATCH_SIZE
                )
                self.stdout.write(f"Imported {i}/{total} leagues...")
                leagues_to_create = []

        self.stdout.write(
            self.style.SUCCESS(f"Finished importing {len(leagues_to_create)} leagues")
        )
