...
function initLoadMorePredictionsButton() {
  ...
  button.addEventListener("click", function () {
    ...
    const url = `/api/history/predictions/?page=${nextPage}`;
    ...
    fetch(url)
      ...
      .then((data) => {
        data.results.forEach((prediction) => {
          ...
          <div class="${prediction.status == "LOST" ? "bg-red-500/10 border-red-500/50 hover:border-red-500" : "bg-[#14212e] border-emerald-500/50 hover:border-emerald-500"} backdrop-blur-sm rounded-lg p-6 border border-primary-700/30  transition-all duration-300 group transform animate-fadeIn">
            ...
            ${
              prediction.detailed_analysis !== ""
               ...
               class="text-secondary-400 flex items-center gap-2 group group-hover:text-primary-300 transition-all duration-300">
                <svg class="w-5 h-5"><use xlink:href="/static/assets/svg/sprite6.svg#arrowRight"></use></svg>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-2">
            ...
            ${prediction.status == "LOST" ? '<div class="flex items-center justify-end gap-2"><p class="text-red-500 font-bold">LOST</p> <svg class="w-5 h-5 text-red-500"><use xlink:href="/static/assets/svg/sprite6.svg#circleX"></use></svg></div>' : '                <div class="flex items-center justify-end gap-2"><p class="text-emerald-500 font-bold">WON</p><svg class="w-5 h-5 text-emerald-500"><use xlink:href="/static/assets/svg/sprite6.svg#circleCheck"></use></svg></div>'}
        ...
        <div class="bg-[#0D151E] border-primary-700/50 rounded-lg border p-4 shadow-lg relative group-hover:border-secondary-500/30 transition-all duration-300 flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center justify-end gap-1">
                        <svg class="w-5 h-5 text-emerald-400"><use xlink:href="/static/assets/svg/sprite6.svg#chevronUp"></use></svg>
