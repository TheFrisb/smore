...
from rest_framework.status import HTTP_422_UNPROCESSABLE_ENTITY, HTTP_200_OK, HTTP_204_NO_CONTENT
from rest_framework.views import APIView

from accounts.models import User, PurchasedPredictions, PlatformType, PurchasedTickets, PurchasedDailyOffer, \
    UserSubscription
...
class ConsumablePurchaseWebhookView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data['type'] != 'NON_RENEWING_PURCHASE':
            logger.warning(
                'Received unsupported purchase type: %s',
                data['type']
            )
            return Response({'detail': 'Unsupported purchase type'}, status=HTTP_422_UNPROCESSABLE_ENTITY)

        return Response(
            status=HTTP_200_OK,
            data={'detail': 'Purchase processed successfully'}
        )
...
class ConsumeConsumableView(APIView):
    class InputSerializer(serializers.Serializer):
        ...
        consumable_type = serializers.ChoiceField(
            choices=["single_prediction", "single_ticket", "daily_offer"],
            required=True
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not verify_transaction(original_transaction_id, self.request.user.id):
            logger.error(
                ...
                self.request.user.id, original_transaction_id
            )
            return Response(
        ...
        if consumable_type == "single_prediction":
            ...
            if PurchasedPredictions.objects.filter(
                ...
                logger.warning(
                    ...
                    original_transaction_id, self.request.user.username
                )
                return Response(
            ...
            logger.info(
                ...
                purchased_prediction.prediction.match, self.request.user.username
            )
            ...
            logger.info(
                ...
                purchased_ticket.ticket, self.request.user.username
            )
        else:
...
class SubscriptionWebhookView(APIView):
    ...
    def get_user_subscriptions(self, app_user_id):
        try:
            return UserSubscription.objects.get(user_id=int(app_user_id),
