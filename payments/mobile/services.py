...
def verify_transaction(transaction_id, app_user_id):
    ...
    customer_id = int(purchase.get('customer_id')) if purchase.get('customer_id') else None

    logger.info(f'CustomerID: {customer_id} for transaction {transaction_id} and user {app_user_id}')
